...
def list_questions(
    ...
    page = max(1, page)
    where, values = _build_question_filters(user, modal_type, logic_system, difficulty, question_type, review_status, source)

    total_row = fetch_one(f"SELECT COUNT(*) AS count FROM questions WHERE {where}", values)
    total = total_row["count"] if total_row else 0
    total_pages = max(1, (total + page_size - 1) // page_size)
    offset = (page - 1) * page_size

    rows = fetch_all(
        f"SELECT * FROM questions WHERE {where} ORDER BY updated_at DESC, id DESC LIMIT ? OFFSET ?",
...
def question_stats(user: dict = Depends(get_current_user)) -> dict[str, Any]:
    """
    返回当前用户（或全部，若为管理员）题目的分类统计信息，
    用于首页数据卡片和图表，无需加载全量题目列表。
    """
    clause, params = visible_owner_clause(user)
    rows = fetch_all(
        f"""
        SELECT modal_type, difficulty, logic_system, review_status, COUNT(*) AS count
        FROM questions WHERE {clause}
        GROUP BY modal_type, difficulty, logic_system, review_status
        """,
        params,
    )
    by_modal: dict[str, int] = defaultdict(int)
    by_difficulty: dict[str, int] = defaultdict(int)
    by_logic: dict[str, int] = defaultdict(int)
    by_status: dict[str, int] = defaultdict(int)
    for row in rows:
        by_modal[row["modal_type"]] += row["count"]
        by_difficulty[row["difficulty"]] += row["count"]
        by_logic[row["logic_system"]] += row["count"]
        by_status[row["review_status"]] += row["count"]
    total_row = fetch_one(f"SELECT COUNT(*) AS count FROM questions WHERE {clause}", params)
    return {
        "total": total_row["count"] if total_row else 0,
        "by_modal_type": dict(by_modal),
        "by_difficulty": dict(by_difficulty),
        "by_logic_system": dict(by_logic),
...
def export_questions(
    ...
    """
    按当前筛选条件导出题目，支持 JSON 和 CSV 两种格式。
    与评测报告导出接口保持一致的下载机制。
    """
    where, values = _build_question_filters(user, modal_type, logic_system, difficulty, question_type, review_status, source)
    rows = fetch_all(
        f"SELECT * FROM questions WHERE {where} ORDER BY updated_at DESC, id DESC",
        values,
    )
    items = [row_to_question(row) for row in rows]
    ...
    output = io.StringIO()
    fieldnames = [
        "id", "title", "question_text", "answer", "question_type",
        "modal_type", "logic_system", "modal_depth", "difficulty",
        "review_status", "source", "explanation",
    ]
...
@app.post("/api/questions/batch-confirm")
def batch_confirm_questions(payload: BatchConfirmIn, user: dict = Depends(get_current_user)) -> dict[str, Any]:
    """
    批量确认题目：将草稿状态改为已确认。
    跳过无权访问或不存在的题目 ID，返回实际确认数量。
    """
    confirmed_ids: list[int] = []
    skipped: list[int] = []
    for qid in payload.question_ids:
        try:
            ensure_owned_question(qid, user)
            with get_connection() as conn:
                conn.execute(
                    "UPDATE questions SET review_status = 'confirmed', updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                    (qid,),
                )
                conn.commit()
            confirmed_ids.append(qid)
        except HTTPException:
            skipped.append(qid)
    return {"confirmed": len(confirmed_ids), "ids": confirmed_ids, "skipped": skipped}
...
def recompute_reports(eval_task_id: int) -> None:
    predictions = [dict(row) for row in fetch_all(
        "SELECT p.*, q.modal_type, q.logic_system, q.difficulty, q.question_type FROM eval_predictions p JOIN questions q ON q.id = p.question_id WHERE p.eval_task_id = ?",
        (eval_task_id,),
    )]
...
def run_eval_task(eval_task_id: int, dataset_id: int, models: list[dict[str, str]], strategies: list[dict[str, Any]]) -> None:
    try:
        ...
        with get_connection() as conn:
            conn.execute(
                "UPDATE eval_tasks SET status = 'running', total_questions = ?, finished_questions = 0 WHERE id = ?",
                (len(questions) * len(models) * len(strategies), eval_task_id),
            )
            conn.commit()
        ...
        for model in models:
            for strategy in strategies:
                template = fetch_one(
                    ...
                    (strategy.get("prompt_template_id") or {"zero_shot": 1, "cot": 2, "few_shot": 3}[strategy["strategy_type"]],),
                )
        ...
        with get_connection() as conn:
            conn.execute(
                "UPDATE eval_tasks SET status = 'completed', finished_at = CURRENT_TIMESTAMP WHERE id = ? AND status != 'cancelled'",
                (eval_task_id,),
            )
            conn.commit()
    except Exception:
...
def cancel_eval_task(task_id: int, user: dict = Depends(get_current_user)) -> dict[str, Any]:
    ...
    with get_connection() as conn:
        conn.execute(
            "UPDATE eval_tasks SET status = 'cancelled', finished_at = CURRENT_TIMESTAMP WHERE id = ? AND status IN ('pending', 'running')",
            (task_id,),
        )
        conn.commit()
    return get_eval_task(task_id, user)
...
def export_eval_report(task_id: int, format: Literal["json", "csv"] = "json", user: dict = Depends(get_current_user)):
    ...
    writer = csv.DictWriter(
        output,
        fieldnames=["id", "question_id", "model_name", "strategy_type", "parsed_answer", "gold_answer", "is_correct", "status", "error_message"],
    )
    writer.writeheader()
...
def admin_activity_log(limit: int = 20, _: dict = Depends(require_admin)) -> list[dict[str, Any]]:
    """
    返回最近系统活动事件流，合并题目、数据集、评测任务三类事件。
    无需新建日志表，基于现有 created_at 字段实现。
    """
    limit = max(1, min(limit, 100))
    q_rows = fetch_all(
        """
        SELECT 'question' AS type, u.username,
               '新增题目：' || q.title AS description,
    ...
    ds_rows = fetch_all(
        """
        SELECT 'dataset' AS type, u.username,
               '创建数据集：' || d.name AS description,
    ...
    task_rows = fetch_all(
        """
        SELECT 'eval_task' AS type, u.username,
               '发起评测：' || t.task_name AS description,
    ...
    all_events = [dict(row) for row in list(q_rows) + list(ds_rows) + list(task_rows)]
    all_events.sort(key=lambda x: x["time"] or "", reverse=True)
